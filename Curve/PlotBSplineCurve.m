...
for ii = 1:size(uksi, 1)
    plot(uksi(ii, :), usfv(ii, :), 'color', "k", 'LineWidth', 1.0)
    hold on
end
axis([0 1 0 1])
xlabel('\fontname{Courier} \xi') 
subtitle({'\fontname{Courier} Polynomial Degree = 3'; ...
    '\fontname{Courier} \xi = [0,0,0,0.33,0.66,1,1,1]'})
title('\fontname{Courier} Shape Functions')
...
plot(cps(1,:), cps(2,:),"--o" , "color", "k",...
    "MarkerFaceColor", "k", 'LineWidth', 0.25)
hold on
plot(curve(1, :), curve(2, :), "k", 'LineWidth',0.75)
title('\fontname{Courier} Random B-spline Curve')
subtitle({'\fontname{Courier} Polynomial Degree = 3'; ...
    '\fontname{Courier} \xi = [0,0,0,0.33,0.66,1,1,1]'})
xlabel('\fontname{Courier} x') 
ylabel('\fontname{Courier} y')
